...
namespace Commando
{
    class EngineStateOptions : EngineStateInterface
    {
        ...
        const float OPTIONS_MENU_SPACING = 50.0f;
        ...
        public EngineStateInterface update(Microsoft.Xna.Framework.GameTime gameTime)
        {
                ...
                inputs.getButton(InputsEnum.BUTTON_1))
            {
                ...
                {
                    case STR_MOVEMENT_TYPE_ABSOLUTE:
                    case STR_MOVEMENT_TYPE_RELATIVE:
                        Settings.getInstance().swapMovementType();
                        if (Settings.getInstance().getMovementType() == MovementType.ABSOLUTE)
                        {
                            menuList_.setString(currentPos, STR_MOVEMENT_TYPE_ABSOLUTE);
                        }
                        else
                        {
                            menuList_.setString(currentPos, STR_MOVEMENT_TYPE_RELATIVE);
                        }
                        break;
